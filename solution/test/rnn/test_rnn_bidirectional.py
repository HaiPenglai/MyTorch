...
class TestCustomBidirectionalRNN(unittest.TestCase):
    def setUp(self):
        ...
        self.rnn = nn.RNN(input_size=self.input_size, 
                         ...
                         batch_first=True,
                         bidirectional=True)
        torch.save(self.rnn.state_dict(), 'bidirectional_rnn.pth')
        
        self.custom_rnn = CustomBidirectionalRNN(input_size=self.input_size,
                                               hidden_size=self.hidden_size,
                                               num_layers=self.num_layers)
        self.custom_rnn.load_state_dict(torch.load('bidirectional_rnn.pth', weights_only=True))
        
        self.x = torch.randn(self.batch_size, self.seq_len, self.input_size)
        
    def test_CustomBidirectionalRNN_loading(self):
    ...
    def test_CustomBidirectionalRNN_forward(self):
